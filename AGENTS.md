...
6. **PlexPulse** → detects "Available" status in `media-status.ts` — usually within about a minute via the event-driven `POST /api/webhooks/arr-import` fast path's live Plex check with bounded retry, falling back to the 24h scheduled scan (`lib/scan-scheduler.ts`) or a manual admin rescan if Plex hasn't scanned the file in yet by then (see SETUP.md)
