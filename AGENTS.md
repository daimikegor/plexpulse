...
- **Plex TV GUIDs are episode-level, not series-level, in "recently added" listings**: `checkPlexLibraryLive()` in `lib/plex-library.ts` (used by the arr-import webhook's instant-availability check) queries `/library/sections/{key}/recentlyAdded` for a fast, targeted lookup instead of a full scan. For TV sections this endpoint returns *episodes*, and an episode's own `Guid[]` holds the *episode's* TMDB/TVDB id — not the series'. Confirmed live against a real Plex server (an X-Men '97 episode's GUID was unrelated to the series' own TMDB id). Matching a series-level tmdbId directly against episode Guids will never match. The fix: resolve each unique episode's parent show via its `grandparentRatingKey` and a follow-up `/library/metadata/{ratingKey}?includeGuids=1` call to get the series' own Guid. Do not "simplify" this back to matching episode Guids directly — it silently breaks TV availability detection.
- **The arr-import webhook's live-check retry window is sized for Plex's real scan latency, not `recentlyAdded` churn**: the original ~60s bounded retry (`LIVE_CHECK_DELAYS_MS` in `app/api/webhooks/arr-import/route.ts`) missed a real import in production — a 1080p movie imported into a large section (~971 items) wasn't caught, while the 4K version of the same title, imported into a much smaller section (~43 items), was caught immediately. `checkPlexLibraryLive()`'s section-iteration was confirmed correct (it checks every section of the relevant type, every attempt — verified by reading the code and live-testing directly against the section that missed). The actual cause: Plex itself hadn't finished scanning the file into its catalog yet within 60s — larger/busier sections can take Plex meaningfully longer to index a new file via Autopulse's triggered partial scan. Extended to ~5.3 minutes (0/+15s/+45s/+90s/+180s) to give that latency realistic room. If a future report says the live check "isn't working," check whether the item simply needed more time (still within the 5.3min window) before assuming a code regression.
...
6. **PlexPulse** → detects "Available" status in `media-status.ts` — usually within a few minutes via the event-driven `POST /api/webhooks/arr-import` fast path's live Plex check with bounded retry (~5.3min, tuned to real-world Plex scan latency — see SETUP.md), falling back to the 24h scheduled scan (`lib/scan-scheduler.ts`) or a manual admin rescan if Plex hasn't scanned the file in yet by then
